...
func NewElastic() (*elasticsearch.Client, error) {
	cfg := ElasticConfig{
		Address:  env.Lookup("ES_HOST", "localhost:9200"),
		Username: env.Lookup("ES_USERNAME", "elastic"),
		Password: env.Lookup("ES_PASSWORD", "35411231"),
		CloudID:  env.Lookup("ES_CLOUD_ID", ""),
		APIKey:   env.Lookup("ES_API_KEY", ""),
	}
